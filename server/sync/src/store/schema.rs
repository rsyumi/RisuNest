...
CREATE TABLE records (domain TEXT NOT NULL, key TEXT NOT NULL, version TEXT NOT NULL, PRIMARY KEY(domain,key));
CREATE TABLE record_relations (domain TEXT NOT NULL, source TEXT NOT NULL, target TEXT NOT NULL, PRIMARY KEY(domain,source,target), FOREIGN KEY(domain,source) REFERENCES records(domain,key));
CREATE INDEX record_relations_target ON record_relations(domain,target);
CREATE TABLE record_scopes (domain TEXT NOT NULL, key TEXT NOT NULL, scope TEXT NOT NULL, PRIMARY KEY(domain,key,scope), FOREIGN KEY(domain,key) REFERENCES records(domain,key));
CREATE INDEX record_scopes_members ON record_scopes(scope,domain,key);
...
CREATE TABLE staged_records (stage TEXT NOT NULL REFERENCES staged_changes(id) ON DELETE CASCADE, domain TEXT NOT NULL, key TEXT NOT NULL, body TEXT NOT NULL, PRIMARY KEY(stage,domain,key));
CREATE TABLE staged_fences (stage TEXT NOT NULL REFERENCES staged_changes(id) ON DELETE CASCADE, domain TEXT NOT NULL, key TEXT NOT NULL, body TEXT NOT NULL, PRIMARY KEY(stage,domain,key));
