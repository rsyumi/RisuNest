...
const english = {
    ...
    syncHelp: 'Several devices share the same characters and chats. A synchronization repository contains characters, chats and attachments only.',
    ...
    secretsLater: 'Passwords and keys are entered in the next step.', signInLater: 'You sign in during the next step.',
    scope: 'Included in backups', library: 'Characters, chats and attachments', deviceSettings: 'Settings of this device', devicePlugins: 'Plugin data of this device',
    prepare: 'Review details',
    pendingVerification: 'The next step shows a summary of what you entered. The server is contacted only after you press Connect.',
    sequentialTitle: 'Use one device at a time',
    sequentialWarning: 'Confirm that synchronization finished on one device and close the app before opening another. Using two devices at once can pick the other device\'s content as latest, and you may need to restore from history.',
    backupWarningTitle: 'A backup-only repository never restores by itself',
    backupWarning: 'It only uploads the data of this device. Changes uploaded from other devices are not brought to this device, and using the repository contents means restoring from history yourself.',
    ...
    recoveryRequired: 'The repository is encrypted. Enter the contents of the recovery file (or the QR code) saved when it was created, together with the recovery code.',
    ...
    preservationComplete: 'Both sides are kept in history. Choose which one to keep. The other side can still be restored from history.',
    ...
    fields: {
        ...
        's3.accountId': 'Account name', 's3.bucket': 'Bucket', 's3.prefix': 'Folder', 's3.region': 'Region', 's3.addressing': 'Addressing', 's3.accessKeyId': 'Access key ID', 's3.secretAccessKey': 'Secret access key',
        'google_drive.accountId': 'Account', 'google_drive.folderId': 'Folder ID', 'google_drive.space': 'Storage location', 'google_drive.oauthRedirectUri': 'Sign-in completion address (HTTPS)', 'google_drive.projectId': 'OAuth project ID', 'google_drive.clientId': 'OAuth client ID for this device',
        'onedrive.accountId': 'Account', 'onedrive.accountType': 'Account type', 'onedrive.tenant': 'Tenant', 'onedrive.driveId': 'Drive ID', 'onedrive.rootItemId': 'Folder item ID', 'onedrive.redirectUri': 'Sign-in completion address', 'onedrive.projectId': 'App client ID', 'onedrive.clientId': 'Client ID for this device',
        'mybox.accountId': 'Account name', 'mybox.rootFolderName': 'Folder name', 'mybox.rootFolderId': 'Existing folder ID', 'mybox.pat': 'Personal access token', 'mybox.expiresAtMs': 'Token expiry',
        'github_releases.accountId': 'GitHub user name', 'github_releases.uploadEndpoint': 'Upload address', 'github_releases.owner': 'Owner', 'github_releases.repo': 'Private repository name', 'github_releases.tagPrefix': 'Tag prefix', 'github_releases.token': 'Personal access token (fine-grained)',
        'gitlab_packages.accountId': 'Token owner', 'gitlab_packages.projectId': 'Project ID or path', 'gitlab_packages.packageName': 'Package name', 'gitlab_packages.maxFileBytes': 'Maximum file size (bytes)', 'gitlab_packages.token': 'Access token', 'gitlab_packages.tokenKind': 'Token type',
    },
    fieldHelp: {
        ...
        'google_drive.accountId': 'Filled in after signing in', 'onedrive.accountId': 'Filled in after signing in',
...
const korean: typeof english = {
    ...
    syncHelp: '여러 기기가 같은 캐릭터·대화를 씁니다. 동기화 저장소는 캐릭터·대화와 첨부 파일만 포함합니다.',
    ...
    secretsLater: '비밀번호와 키는 다음 단계에서 입력합니다.', signInLater: '로그인은 다음 단계에서 합니다.',
    scope: '백업에 포함', library: '캐릭터·대화와 첨부 파일', deviceSettings: '이 기기의 설정', devicePlugins: '이 기기의 플러그인 데이터',
    prepare: '입력 내용 확인',
    pendingVerification: '다음 단계에서 입력한 내용을 정리해 보여드립니다. 서버에 실제로 연결하는 것은 연결 버튼을 누른 뒤입니다.',
    sequentialTitle: '한 번에 한 기기에서만 쓰세요',
    sequentialWarning: '한 기기에서 동기화가 끝난 것을 확인하고 앱을 닫은 뒤 다른 기기를 여세요. 동시에 쓰면 다른 기기의 내용이 최신으로 선택될 수 있고, 이력에서 직접 복원해야 할 수 있습니다.',
    backupWarningTitle: '백업만 하는 저장소는 자동으로 되돌리지 않습니다',
    backupWarning: '이 기기의 데이터를 올려 두기만 합니다. 다른 기기에서 올린 변경을 이 기기에 가져오지 않으며, 저장소의 내용을 쓰려면 이력에서 직접 복원해야 합니다.',
    ...
    recoveryRequired: '암호화된 저장소입니다. 이 저장소를 만들 때 저장한 복구 파일의 내용(또는 QR 내용)과 복구 코드를 입력하세요.',
    ...
    preservationComplete: '양쪽 내용을 모두 이력에 보관했습니다. 어느 쪽을 남길지 고르세요. 고르지 않은 쪽도 이력에서 되돌릴 수 있습니다.',
    ...
    fields: {
        ...
        's3.accountId': '계정 이름', 's3.bucket': '버킷', 's3.prefix': '폴더', 's3.region': '리전', 's3.addressing': '주소 방식', 's3.accessKeyId': '액세스 키 ID', 's3.secretAccessKey': '시크릿 액세스 키',
        'google_drive.accountId': '계정', 'google_drive.folderId': '폴더 ID', 'google_drive.space': '저장 위치', 'google_drive.oauthRedirectUri': '로그인 완료 주소 (HTTPS)', 'google_drive.projectId': 'OAuth 프로젝트 ID', 'google_drive.clientId': '이 기기용 OAuth 클라이언트 ID',
        'onedrive.accountId': '계정', 'onedrive.accountType': '계정 종류', 'onedrive.tenant': '테넌트', 'onedrive.driveId': '드라이브 ID', 'onedrive.rootItemId': '폴더 항목 ID', 'onedrive.redirectUri': '로그인 완료 주소', 'onedrive.projectId': '앱 클라이언트 ID', 'onedrive.clientId': '이 기기용 클라이언트 ID',
        'mybox.accountId': '계정 이름', 'mybox.rootFolderName': '폴더 이름', 'mybox.rootFolderId': '기존 폴더 ID', 'mybox.pat': '개인 액세스 토큰', 'mybox.expiresAtMs': '토큰 만료 시각',
        'github_releases.accountId': 'GitHub 사용자 이름', 'github_releases.uploadEndpoint': '업로드 주소', 'github_releases.owner': '소유자', 'github_releases.repo': '비공개 저장소 이름', 'github_releases.tagPrefix': '태그 접두어', 'github_releases.token': '개인 액세스 토큰 (fine-grained)',
        'gitlab_packages.accountId': '토큰 소유자', 'gitlab_packages.projectId': '프로젝트 ID 또는 경로', 'gitlab_packages.packageName': '패키지 이름', 'gitlab_packages.maxFileBytes': '파일 최대 크기 (바이트)', 'gitlab_packages.token': '액세스 토큰', 'gitlab_packages.tokenKind': '토큰 종류',
    },
    fieldHelp: {
        'webdav.root': '이 폴더 안에 저장소를 만듭니다. 다른 파일과 함께 두지 마세요.',
        'webdav.password': '서비스 설정에서 만든 앱 비밀번호입니다. 계정 비밀번호가 아닙니다.',
        'google_drive.accountId': '로그인 후 채워집니다', 'onedrive.accountId': '로그인 후 채워집니다',
