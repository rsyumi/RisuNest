...
const HANDOFF_KIND_TO_JOB_KIND: Record<string, keyof typeof ANDROID_SAF_HANDOFF_ID_PATTERNS> = {
...
describe('native file taxonomy golden fixture', () => {
    it('covers every recovery pattern with a fixture grammar and vice versa', () => {
        expect(Object.values(HANDOFF_KIND_TO_JOB_KIND).sort()).toEqual(
            Object.keys(ANDROID_SAF_HANDOFF_ID_PATTERNS).sort(),
        )
        expect(taxonomy.managedHandoffs.map((grammar) => grammar.kind).sort()).toEqual(
            Object.keys(HANDOFF_KIND_TO_JOB_KIND).sort(),
        )
    })

    it('accepts every managed handoff grammar and captures the export id', () => {
        for (const grammar of taxonomy.managedHandoffs) {
            const pattern = ANDROID_SAF_HANDOFF_ID_PATTERNS[
            ...
            for (const suffix of grammar.suffixes) {
                ...
                expect(pattern!.exec(`/data/app/${taxonomy.handoffDirectory}/${name}`)?.[1], name)
                    .toBe(taxonomy.uuid)
            }
        }
    })
    ...
    it('rejects malformed managed module names', () => {
        ...
        for (const name of taxonomy.rejectedModuleNames) {
            expect(pattern.exec(`/data/app/${taxonomy.handoffDirectory}/${name}`), name).toBeNull()
        }
    })
    ...
    it('rejects a grammar name against every other grammar', () => {
        for (const grammar of taxonomy.managedHandoffs) {
            ...
            for (const other of taxonomy.managedHandoffs) {
                ...
                expect(pattern.exec(`/x/${name}`), `${name} vs ${other.kind}`).toBeNull()
            }
        }
    })
    ...
    it('routes database spool suffixes to restore and content suffixes to unsupported', async () => {
        const suffixes = [
            ...taxonomy.spoolSuffixes.database.map((suffix) => [suffix, 'restore'] as const),
            ...taxonomy.spoolSuffixes.content.map((suffix) => [suffix, 'unsupported'] as const),
        ]
        for (const [suffix, route] of suffixes) {
            const restore = vi.fn(async () => {})
            const unsupported = vi.fn()
            await consumeAndroidSpoolBatch(
                { requestId: 'request-1', ready: [spoolSource(`import${suffix}`)], failures: [] },
                { restore, unsupported },
            )
            expect(restore.mock.calls.length, suffix).toBe(route === 'restore' ? 1 : 0)
            expect(unsupported.mock.calls.length, suffix).toBe(route === 'unsupported' ? 1 : 0)
        }
    })

    it('classifies exactly the content suffixes as native content spools', () => {
        for (const suffix of taxonomy.spoolSuffixes.content) {
            expect(isAndroidNativeContentSpool(spoolSource(`a${suffix}`)), suffix).toBe(true)
            expect(
                isAndroidNativeContentSpool(spoolSource(`a${suffix.toUpperCase()}`)),
                suffix,
            ).toBe(true)
        }
        for (const suffix of taxonomy.spoolSuffixes.database) {
            expect(isAndroidNativeContentSpool(spoolSource(`a${suffix}`)), suffix).toBe(false)
        }
