export const languageChineseTraditional = {
    ...
    "errors": {
        ...
        "networkFetchWeb": "這可能是由 CORS 錯誤引起的。這種情況只會在使用網頁版時發生，因為瀏覽器存在一些限制。請嘗試使用桌面版或其他版本的 RisuNest。",
    ...
    "help": {
        ...
        "streamingDisplayOptimizationMode": "在使用正規表示式腳本等大量後處理時，減少長回覆串流生成造成的介面卡頓。對行動裝置或低階裝置可能有幫助。\n\n精確相容會處理提供方的每次更新。相容性最高，CPU 成本也最高。\n\n均衡會立即顯示第一次更新，之後約每 0.125 秒只處理最新更新。有狀態腳本的執行次數可能減少。\n\n強會批次顯示原始預覽，並僅在正常完成時進行一次語意後處理。它對中間行為的改變最大。\n\n所選模式會固定到目前生成結束。均衡或強可能改變有狀態正規表示式、Lua 或外掛操作的執行次數。",
        ...
        "antiClaudeOverload": "當 Claude 發生過載時，RisuNest 會嘗試繼續發送相同的提示詞以防止請求中斷，藉此減少過載機率。此功能僅適用於串流回應 (Streamed Responses)，對非官方 API 端點可能無效。",
        ...
        "defaultVariables": "可在此處設定自訂預設變數。使用 `<variable name>=<variable value>` 格式，每行一個。例如：`name=RisuNest`，可在觸發式和 CBS 變數中使用，如：`{{getvar::A}}`、`{{setvar::A::B}}` 或 `{{? $A + 1}}`。若提示詞模板的預設變數與角色的預設變數名稱相同，系統將使用角色的預設變數。",
    ...
    "setup": {
        ...
        "welcome": "歡迎使用 RisuNest！我將引導您進行設定。請問我該如何稱呼您？",
    ...
    "streamingDisplayOptimizationOff": "精確相容",
    "streamingDisplayOptimizationBalanced": "均衡（建議）",
    ...
    "officialWikiDesc": "歡迎查看 RisuNest 官方的 Wiki。",
    "officialDiscord": "官方 Discord",
    "officialDiscordDesc": "RisuNest 官方的 Discord 伺服器",
    "persona": "使用者設定",
    "icon": "圖示",
    "account": "帳號",
    "remove": "刪除",
    ...
    "restoreLocalSnapshot": "還原本機快照",
    "chooseLocalSnapshot": "選擇本機快照",
    "noLocalSnapshots": "沒有可用的本機快照。",
    ...
    "shareExport": "分享/匯出",
    "risupresetDesc": "Risupreset 是專為 RisuNest 預設設計的格式。",
    "risuMDesc": "RisuM 是專為 RisuNest 模組設計的格式。",
    ...
    "pluginVersionWarn": "這是 {{plugin_version}} 版本的外掛，與當前 RisuNest 版本不相容。請更新外掛至 {{required_version}} 版本。",
    ...
    "pluginRiskDetectedAlert": "RisuNest 偵測到可能有害的程式碼，該外掛可能**竊取您的 API 金鑰或個人資料**。強烈建議**不要安裝此外掛**。",
    "doNotInstall": "不要安裝",
    "continueAnyway": "無論如何都要繼續",
    "pluginNoRiskButAlert": "RisuNest 未偵測到有害的程式碼，但請注意，從不受信任的來源安裝外掛可能存在安全風險。",
    ...
    "enableRisuaiProTools": "啟用 RisuNest Pro Tools",
    ...
    screenshotTurns: "總輪數：{total}",
    screenshotInclusiveRange: "輪次範圍（從 1 開始，包含起止）",
    screenshotStart: "開始",
    screenshotEnd: "結束",
    screenshotRecent50: "最近 50 輪",
    screenshotFull: "全部",
    screenshotCapture: "截取",
    screenshotProgress: "已完成 {completed}/{total} 輪",
    screenshotEmpty: "沒有可截取的對話輪次。",
    screenshotIntegerRange: "請輸入整數輪次。",
    screenshotRangeBounds: "輪次必須在目前對話範圍內。",
