...
fn decode_cold_payload(source: &Path, payload: &F0PayloadDescriptor) -> Result<Value, F0Error> {
    ...
    let value: Value = serde_json::from_reader(&mut limited).map_err(|error| F0Error {
        ...
        message: if limited.exceeded {
            format!("F0 cold payload exceeds the decoded limit: {}", payload.key)
        } else {
            ...
            .is_some_and(|value| value.contains_key("character") || value.contains_key("message"))
    {
        return Err(F0Error {
            code: F0ErrorCode::ColdPayload,
            message: format!("unsupported F0 cold payload value: {}", payload.key),
        });
    }
    Ok(value)
}
    ...
    fn read(&mut self, output: &mut [u8]) -> std::io::Result<usize> {
        ...
        if self.remaining == 0 {
            ...
            return Err(std::io::Error::new(
                ...
                "decoded F0 cold payload limit exceeded",
            ));
        }
