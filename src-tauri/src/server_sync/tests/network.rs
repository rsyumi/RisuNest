...
fn proxy_413_429_524_resume_only_unverified_chunks_and_lost_accept_is_not_resent() {
    for code in [413, 429, 524] {
        run_case(0, Duration::ZERO, code, false);
    }
    run_case(0, Duration::ZERO, 524, true);
}
#[test]
...
fn slow_network_full_transfer_gate() {
    for (bits, millis) in [(10_000_000, 80), (1_000_000, 200)] {
        let result = run_case(bits / 8, Duration::from_millis(millis), 0, false);
        eprintln!("synthetic bits/sec={bits}, request delay={millis}ms: (HTTP bytes, requests, upload ms, download ms)={result:?}");
