use super::{current_revision, PersistentStore, StoreError, StoreResult};
...
impl PersistentStore {
    ...
    pub(crate) fn forget_sync_device(
        ...
        if device_updated != 1 || common_deleted != 1 {
            ...
            .map(|(record_key, deleted_generation_sequence)| {
                ...
                let blocking_device_ids = devices
                    ...
                    .filter(|device| match device.status {
                        RegisteredSyncDeviceStatus::Active => {
                            compare_sequences(
                                &device.acknowledged_generation.generation_sequence,
        ...
        "SELECT (
            ...
            WHERE device.library_id = ?1
              AND (
                ...
                OR (device.status = 'forgotten' AND common_base.peer_id IS NOT NULL)
              )
        ) + (
            SELECT COUNT(*)
