#[cfg(test)]
use crate::asset_repository::owner_manifest_codec::decode_owner_manifest;
use super::record_projection::*;
use super::{
...
use crate::{
    ...
    peer_sync::logical_delta::{
        ...
        IndexedLogicalRecord, LogicalAssetAliasMetadata, LogicalManifestObject,
        LogicalRecordEnvelope, LogicalRecordLocator, LOGICAL_MESSAGE_PAGE_SIZE,
    },
};
...
pub(super) fn reconstruct_record(
    ...
    let page_hashes = if matches!(
        decode_logical_record_key(record_key).map_err(codec_error)?,
        LogicalRecordLocator::Conversation { .. }
    ) {
        let mut statement = connection.prepare("SELECT object_hash FROM logical_message_page_sources WHERE library_id=?1 AND generation_id=?2 AND record_key=?3 ORDER BY page_index ASC")?;
