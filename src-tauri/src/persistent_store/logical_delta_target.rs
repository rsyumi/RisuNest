...
use crate::{
    ...
    peer_sync::{
        logical_delta::{
            ...
            LogicalOwnerHead, LogicalRecordEnvelope, LogicalRecordLocator,
...
use super::record_apply::ResolvedOwnerHead;
fn record_error(error: StoreError) -> PeerSyncError {
    match error {
...
fn apply_put(
    ...
    let count = put.pages.iter().try_fold(0_u64, |count, page| {
        count
            .checked_add(page.message_count)
            .ok_or_else(|| PeerSyncError::Validation("logical message count overflow".to_owned()))
    })?;
    super::record_apply::apply_record_rows(
        transaction,
        generation,
        &put.key,
        &put.locator,
...
pub(super) fn validate_locator_envelope(
    ...
    super::record_apply::validate_locator_envelope(locator, envelope).map_err(record_error)
}
pub(super) fn rehydrate_root_owners(
    value: &mut Value,
    heads: &[ResolvedOwnerHead],
) -> Result<(), PeerSyncError> {
    super::record_apply::rehydrate_root_owners(value, heads).map_err(record_error)
}
pub(super) fn rehydrate_character_owner(
    ...
    super::record_apply::rehydrate_character_owner(detail, character_id, heads)
        .map_err(record_error)
}
pub(super) fn validate_configured_index_uniqueness(
    ...
    super::record_apply::validate_configured_index_uniqueness(transaction, generation)
        .map_err(record_error)
}
pub(super) fn apply_delete(
    ...
    super::record_apply::apply_delete(transaction, generation, locator).map_err(record_error)
}
