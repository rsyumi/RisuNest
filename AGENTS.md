...
- `pnpm test` runs all root Vitest projects (`app`, `app-extended`, `harness`). `pnpm test:dev [path]` selects ordinary app tests; `pnpm test:watch [path]` watches them. `pnpm test:related <source-path>` is local feedback only, not full acceptance evidence. Manager GUI and registry tests use `pnpm test:manager` and `pnpm test:registry`. `pnpm check:test-inventory` verifies complete, disjoint ownership and requires the independent package installations. `pnpm check` type-checks, `pnpm check:plugin-dts` compiles the plugin API declarations, `pnpm check:production-bundle` builds product bundles and proves they contain no verification code, `pnpm check:benchmark-harnesses` type-checks `benchmarks/`. `pnpm benchmark:phase1` runs the deterministic regex benchmark with fixed output hashes; the other `benchmark:*` scripts are documented in their harness README. Android JVM tests live under `src-tauri/gen/android` (Gradle).
...
- Plugin system (API v3.0 only): iframe sandboxing, SafeDocument/SafeElement wrappers, save-specific and device-specific storage, custom AI providers, hot reload, and scalable API v3 queries. API v2.1 bundles cannot be installed and stored v2.1 records are skipped at load. See `plugins.md` and `src/ts/plugins/migrationGuide.md`.
