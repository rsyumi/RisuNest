...
The fault run waits until the production restore job reports `state=running`, `phase=activating-database`, then terminates only the owned synthetic process. The next process runs the product startup recovery entry before the harness. That broad phase begins before the native restore creates and prepares its durable journal, so recovery may leave either the complete old section set or the complete committed section set, never a mixture, and normal PDS access must reopen. The exact prepared-journal-to-new-state guarantee and individual section and commit-marker crash windows remain deterministic Rust recovery tests because the native file-job status contract does not expose those boundaries.
